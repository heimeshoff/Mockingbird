...
| **Multi-model sidecar** | The python sidecar holds one `TTSModel` per preloaded language in a `_RESIDENT_MODELS` dict (main-039 / ADR 0024). `serve --language english --language german` preloads both concurrently at startup; per-request routing swaps `pocket_tts.main.tts_model` to the matching entry before pocket-tts's `/tts` handler runs. Adding a third language is a config change (extend `--language` flags, restart sidecar), not a code change. **Resident footprint (measured, main-r8k3w / re-measured main-d7m2k):** ~1.12 GB idle with the distilled `german` (torch ~176 MB + pocket_tts ~84 MB runtime; english + distilled german each ~418 MB resident, flow_lm 341 MB apiece). The earlier `german_24l` drift (24-layer preview, flow_lm ~1205 MB, ~2.0 GB idle) was reverted by main-d7m2k — `SidecarHost.cs` and `PocketTtsEngine.LanguageWireValue` both now resolve `german` to the distilled variant, so code, ADR 0025, and this row agree. Inline int8 quantization was measured and rejected as a RAM lever (ADR 0029). |
